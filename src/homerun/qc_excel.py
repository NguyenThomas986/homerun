...
def _tagdir_rows(cfg):
    ...
    for species, sample, leaf_name in sorted(identities):
        ...
        if info.exists():
            ...
            genome_line = next((line for line in lines if line.startswith("genome=")), "")
            ...
            row.update({
                ...
                "Average Tags/Position": _taginfo_value(lines, "averageTagsPerPosition"),
                "Median Tags/Position": _taginfo_value(lines, "medianTagsPerPosition"),
                "Average Read Length": _taginfo_value(lines, "averageTagLength"),
                "Average Fragment GC": _taginfo_value(lines, "averageFragmentGCcontent"),
            })
        rows.append(row)
    return rows
...
def _peak_rows(cfg):
    rows = []
    for species_dir in sorted(path for path in cfg.project.iterdir() if path.is_dir()):
        ...
        for stats in sorted(tss_dir.glob("*.stats.txt")):
            ...
            rows.append({
                ...
                "Sample": sample,
                "Peak call": library,
                "Total csRNA reads": _grab(text, r"Total csRNA reads:\s+([\d.]+)", float),
                "Total input reads": _grab(text, r"Total input reads:\s+([\d.]+)", float),
                "Putative TSS": _grab(text, r"total putative TSS clusters\s+(\d+)", int),
                "Valid TSS": _grab(text, r"Valid TSS clusters\s+(\d+)", int),
                "Distal %": _grab(text, r"Fraction Promoter-Distal.*?:\s+([\d.]+%)", float),
                "Bidirectional %": _grab(text, r"Fraction of bidirectional.*?:\s+([\d.]+%)", float),
                "Stable %": _grab(text, r"Fraction of stable.*?:\s+([\d.]+%)", float),
                "Log2 vs Input": _grab(text, r"log2 fold vs\. input:\s+([\d.\-]+)", float),
                "Log2 vs RNA": _grab(text, r"log2 fold vs\. rna:\s+([\d.\-]+)", float),
...
def _parse_lengths(path):
    ...
    if total <= 0:
        return None
    adapters = sum(count for length, count in values if length == 0)
    retained = total - adapters
    average = (
        sum(length * count for length, count in values if length != 0) / retained
        if retained > 0 else "NA"
    )
    return {
        ...
        "Average Retained Length": round(average, 2) if average != "NA" else "NA",
    }
...
def _parse_star(path):
    ...
    patterns = {
        ...
        "Unmapped (mismatch) %": (
            r"% of reads unmapped: too many mismatches \|\s+([\d.]+)%", float),
    ...
    unmapped = [value for key, value in result.items() if key.startswith("Unmapped (")]
    if unmapped:
    ...
    if all(value is not None for value in (uniquely, multi, too_many)):
        result["Overall Aligned %"] = round(uniquely + multi + too_many, 2)
    return result
...
def _parse_hisat2(path):
    ...
    result = {}
    patterns = {
        "Input Reads": (r"(\d+)\s+reads?;\s+of these", int),
        "Unmapped %": (r"([\d.]+)\s*%\)\s+aligned 0 times", float),
        "Uniquely Mapped %": (r"([\d.]+)\s*%\)\s+aligned exactly 1 time", float),
        "Multi-Mapped %": (r"([\d.]+)\s*%\)\s+aligned >1 times", float),
        "Overall Aligned %": (r"([\d.]+)\s*%\s+overall alignment rate", float),
    }
    for name, (pattern, cast) in patterns.items():
        match = re.search(pattern, text)
        if match:
...
def _trim_alignment_rows(cfg):
    ...
    for species, sample, leaf_name, r1 in iter_leaf_dirs(cfg):
        ...
        lengths = cfg.trimmed_dir(species, sample) / f"{r1.name}.lengths"
        if lengths.exists():
            parsed = _parse_lengths(lengths)
            if parsed:
                trim_rows.append({**common, "Tool": "homerTools", **parsed,
        ...
        if star_log.exists():
            ...
            tool, source = "STAR", star_log
        elif hisat2_log.exists():
            parsed = _parse_hisat2(hisat2_log)
            tool, source = "HISAT2", hisat2_log
        else:
            parsed = None
        if parsed:
            alignment_rows.append({
                **common,
                "Aligner": tool,
                "Input Reads": parsed.get("Input Reads", "NA"),
                "Uniquely Mapped %": parsed.get("Uniquely Mapped %", "NA"),
                "Multi-Mapped %": parsed.get("Multi-Mapped %", "NA"),
                "Unmapped %": parsed.get("Unmapped %", "NA"),
                "Overall Aligned %": parsed.get("Overall Aligned %", "NA"),
                "Source log": str(source),
            })
    return trim_rows, alignment_rows


def _write_table_sheet(workbook, name, rows, table_name):
    ws = workbook.create_sheet(name)
    if not rows:
        ws.append([f"No {name.lower()} data were found."])
    ...
    ws.append(headers)
    for row in rows:
        ws.append([row.get(header, "NA") for header in headers])
...
def _write_overview(workbook, cfg, tag_rows, peak_rows, trim_rows, alignment_rows):
    ws = workbook.create_sheet("Overview")
    ws.append(["HOMERun QC Summary"])
    ws["A1"].fill = HEADER_FILL
    ws["A1"].font = Font(color="FFFFFF", bold=True, size=14)
    ws.merge_cells("A1:B1")
    ws.append(["Project", str(cfg.project)])
    ws.append(["Replicate TagDirs", len(tag_rows)])
    ws.append(["Peak calls", len(peak_rows)])
    ws.append(["Trimming records", len(trim_rows)])
    ws.append(["Alignment records", len(alignment_rows)])
    ws.append(["Species", len({row["Species"] for row in tag_rows})])
    ws.append(["Samples", len({(row["Species"], row["Sample"]) for row in tag_rows})])
...
def write_qc_workbook(cfg):
    ...
    _write_overview(workbook, cfg, tag_rows, peak_rows, trim_rows, alignment_rows)
    _write_table_sheet(workbook, "Trimming", trim_rows, "TrimmingQC")
    _write_table_sheet(workbook, "Alignment", alignment_rows, "AlignmentQC")
    _write_table_sheet(workbook, "Tag Directories", tag_rows, "TagDirectoryQC")
    _write_table_sheet(workbook, "Peak Calling", peak_rows, "PeakCallingQC")
    output = cfg.project / "QC_summary.xlsx"
    workbook.save(output)
    log.info("QC: wrote Excel workbook %s", output)
    return output
